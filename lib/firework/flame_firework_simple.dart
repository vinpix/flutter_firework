...
class FireworkParticleComponent extends PositionComponent {
  ...
  Future<void> onLoad() async {
    ...
    final particle = Particle.generate(
      ...
      count: 200,
      generator: (i) {
        final angle = (i / 200) * 2 * pi;
        final speedMagnitude = 20 + Random().nextDouble() * 200;
        final speed = Vector2(cos(angle), sin(angle)) * speedMagnitude;
        const color = Colors.yellow; // Only yellow color
...
class TrailingPerlinNoiseParticle extends PerlinNoiseParticle {
  ...
  void render(Canvas canvas) {
    ...
    for (int i = 1; i < trail.length; i++) {
      ...
      //Draw the glow effect around the trail line
      // final Paint glowPaint = Paint()
      //   ..color = trailColor.withOpacity(opacity * 1)
      //   ..strokeWidth = 8.0
      //   ..maskFilter = const MaskFilter.blur(BlurStyle.normal, 8);

      // canvas.drawLine(trail[i - 1].toOffset(), trail[i].toOffset(), glowPaint);
    }
  }
}
...
class FadingGlowParticle extends Particle {
  ...
  void render(Canvas canvas) {
    ...
    for (int i = 1; i <= 5; i++) {
      ...
      canvas.drawCircle(Offset.zero, 0, blurredPaint);
    }

    canvas.drawCircle(
        Offset.zero, 0, paint..color = paint.color.withOpacity(currentOpacity));
  }
}
